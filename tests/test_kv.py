import base64
from app.models import AuditLog, KVSecret, KVSecretVersion
...
def test_kv_crud_tamper_and_access(initialized):
    ...
    with TestingSession() as db:
        ...
        version=db.query(KVSecretVersion).one()
        version.ciphertext_b64=("A" if version.ciphertext_b64[0] != "A" else "B")+version.ciphertext_b64[1:]; db.commit()
...
def test_kv_tag_tamper_detected(initialized):
    ...
    with TestingSession() as db:
        row=db.query(KVSecretVersion).one()
...
def test_kv_versioning_keeps_overwrite_history(initialized):
    c=initialized; alice=register_login(c); bob=register_login(c,"bob@example.com")
    path="secret/alice@example.com/versioned"
    first={"password":"first","host":"db1"}
    second={"password":"second","host":"db2"}
    r1=c.put(f"/api/v1/kv/{path}",json={"data":first},headers=alice)
    r2=c.put(f"/api/v1/kv/{path}",json={"data":second},headers=alice)
    assert r1.json()["data"]["version"]==1
    assert r2.json()["data"]["version"]==2
    assert c.get(f"/api/v1/kv/{path}",headers=alice).json()["data"] == {"path":path,"version":2,"value":second}
    assert c.get(f"/api/v1/kv/{path}?version=1",headers=alice).json()["data"]["value"] == first
    versions=c.get(f"/api/v1/kv-versions/{path}",headers=alice).json()["data"]
    assert [item["version"] for item in versions] == [1,2]
    assert c.get(f"/api/v1/kv-versions/{path}",headers=bob).status_code == 403
    with TestingSession() as db:
        row=db.query(KVSecret).one()
        assert db.query(KVSecretVersion).filter_by(kv_secret_id=row.id).count()==2
        assert "first" not in " ".join(v.ciphertext_b64 for v in db.query(KVSecretVersion).all())
