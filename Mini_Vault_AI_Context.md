---
title: "Mini Vault — Assignment 1"
document_type: "computer-security-project-specification"
project: "Mini Vault"
primary_features:
  - "Secure Storage (KV Engine)"
  - "Encryption and Signing as a Service (Transit Engine)"
source_file: "Crypt_proj1(2).docx"
content_language: "English"
agent_purpose: "Implementation, testing, documentation, and demo context"
---

# Mini Vault — Assignment 1

**Course:** Computer Security  
**System type:** Secure vault service  
**Core engines:** KV Engine and Transit Engine

## AI Agent Context Summary

> This section is a derived implementation summary. The complete authoritative assignment specification follows below.

### Project goal

Build a Mini Vault service that:

1. Stores each user's secrets as authenticated ciphertext on disk.
2. Provides encryption, decryption, signing, and verification without exposing key material to clients.
3. Requires vault unlock, user authentication, and ownership checks before protected operations are allowed.

### Mandatory feature set

| ID | Required feature |
|---|---|
| 0.1 | Vault initialization and unlock with a Master Passphrase |
| 0.2 | User registration, login, session expiry, and five-minute account lockout |
| 1.1 | Encrypted-at-rest KV write, read, and delete |
| 1.2 | Ownership-based access control for secret paths |
| 2.1 | Owner-bound named encryption/signing key management |
| 2.2 | Encryption and decryption as a service |
| 2.3 | Ownership-based access control for named keys |
| 2.4 | Digital signing and signature verification as a service |

### Non-negotiable security invariants

- The vault starts in the **locked** state after every process restart.
- The plaintext Data Encryption Key (DEK), named AES keys, and private signing keys must never be written to disk.
- No API may return plaintext or base64-encoded private key material.
- Every Feature 1 and Feature 2 endpoint requires a valid, unexpired session token.
- Authorization must be checked before encryption, decryption, signing, or verification is attempted.
- AES operations must use AEAD, specifically AES-256-GCM, with a fresh nonce for every encryption.
- Modified ciphertext, authentication tags, messages, and signatures must be rejected deterministically.
- Password storage must use bcrypt or Argon2; plain SHA hashing is prohibited.
- Five consecutive login failures lock the account for exactly five minutes.

### Recommended implementation order

1. Storage layer and data models.
2. Vault initialization, unlock, and in-memory DEK lifecycle.
3. Authentication and session validation.
4. KV Engine with ownership enforcement.
5. Named-key storage and ownership enforcement.
6. Transit encrypt/decrypt.
7. Transit sign/verify.
8. Negative tests, tamper tests, README, report, and demo script.

### Recommended technology

- Python with FastAPI or Flask
- SQLite or JSON persistence
- `cryptography` for AES-GCM and asymmetric signing
- `argon2-cffi` or `bcrypt` for password hashing
- Argon2id or PBKDF2-HMAC-SHA256 for deriving the wrapping key
- `pytest` for automated tests

---

> The following sections preserve the complete assignment specification from the source document.
...
- Interface: a CLI is sufficient; a REST API (FastAPI/Flask) is encouraged to better reflect a client calling a "service", similar to the real.
...
> **Technical Hint:** *Refer to the real Transit named-key model: https://developer.hashicorp.com/vault/docs/secrets/transit · KeyUsage concept: https://docs.aws.amazon.com/kms/latest/APIReference/API_CreateKey.html*
...
> **Technical Hint:** *Refer to the real Transit Secrets Engine: https://developer.hashicorp.com/vault/docs/secrets/transit*
...
3. Client calls verify(key_name, message_b64, message_type, signature_b64, token) → the system recomputes/uses the digest the same way as in sign(), checks the signature against the key's public component, and returns a structured result — {key_name, signature_valid, signing_algorithm} — mirroring the response shape of AWS KMS's Verify API (KeyId, SignatureValid, SigningAlgorithm), instead of silently assuming a signature is correct.
...
> **Technical Hint:** *Modeled after AWS KMS: Sign — https://docs.aws.amazon.com/kms/latest/APIReference/API_Sign.html and Verify — https://docs.aws.amazon.com/kms/latest/APIReference/API_Verify.html. Python libraries: cryptography (rsa / ed25519 modules).*
